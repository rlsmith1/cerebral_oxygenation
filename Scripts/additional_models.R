...
# penalized logistic regression -------------------------------------------
  ...
  logistf::logistf(Status ~ ., data = df_model, maxit = , maxstep = )
  ...
  hbox_rec <- recipe(Status ~ ., data = df_model) %>% 
    step_BoxCox(all_predictors()) %>% # or step_normalize()
    step_nzv(all_predictors()) # step_rm() to get rid of what you don't want in model
  
  # prep
  hbox_prep <- hbox_rec %>% prep()
  ...
  # create workflow
  lasso_wf <- workflow() %>% 
    add_recipe(hbox_rec) # workflows know about training data so you don't have to juice
    
  lasso_fit <- lasso_wf %>% 
    add_model(lasso_spec) %>% 
    fit(data = df_model) # still a workflow
  ...
  # How do we figure out how to pick penalty?? --> tune lasso parameters
  
  # resample
  set.seed(234)
  hbox_boot <- bootstraps(df_model, strata = Status)
  
  # new specification
  tune_spec <- logistic_reg(penalty = tune(), mixture = 1) %>% 
    set_engine("glmnet") # use resampling to find right value for this data set
  ...
  lasso_grid %>% 
    collect_predictions(parameters = highest_roc_auc) %>% 
    mutate(Truth = ifelse(Status == 1, "CM", "UM"), Prediction = ifelse(.pred_class == 1, "CM", "UM")) %>% 
  ...
  df_model_split <- df_model %>% mutate(Status_split = case_when(
    
    Status == 1 & Hb_conc_second_alpha > 0.75 ~ "CM_norm",
    Status == 1 & Hb_conc_second_alpha < 0.75 ~ "CM_disrupt",
    Status == 0 ~ "UM"
    
    ))
  
  df_model_split %>% 
    ggplot(aes(avg_Hb_conc, Hb_conc_second_alpha)) +
    geom_point(aes(color = Status_split), size = 2)
  
  df_model_split %>% 
    ggplot(aes(Hematocrit, Hb_conc_second_alpha)) +
    geom_point(aes(color = Status_split, size = avg_Hb_conc))
  
  df_model_split %>% 
    ggplot(aes(Hematocrit, avg_Hb_conc)) +
    geom_point(aes(color = Status_split), size = 2)
  
  df_model_split %>% 
    ggplot(aes(Hb_conc_second_alpha, Hb_o2sat_second_alpha)) +
    geom_point(aes(color = Status_split, size = Hematocrit))
