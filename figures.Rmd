...
#### A. PCA
```{r fig3a}
# include variables of interest (THC, THC alpha, and hematocrit), remove healthy volunteers from analysis
...
# select THC, THC alpha, and hematocrit
  df_hbox_model <- df_hbox_impute %>% 
    dplyr::select(c(2,4:5,19:22)) %>% 
    mutate(`1/10 * THCα` = THCα/10, `1/10 * THC` = THC/10) %>% 
    dplyr::select(-c(THCα, THC)) %>% 
...
### Figure 4: Relationships among THC, THC alpha, and hematocrit
```{r fig4}

df_hbox_impute %>% filter(Status != "HV") %>% 
  
  ggplot(aes(x = THCα, y = `Hematocrit (%)`)) +
  geom_point(aes(color = Status, size = THC), alpha = 0.7) +
  labs(size = 'THC (μM)') +
...
### Supplementary figure 1: Spearman correlations of NIRS variables 
