...
```
**Table 1.** A summary of subject information and hematological data. Group patient data presented as median (range). Note: Sex not recorded for 3 CM patients; Lactate data removed for HV due to inconsistent data.

### Figure 1: Visual summary of NIRS variables

```{r fig1, fig.height = 12}
...
df_hbox_voi_long %>% 
  ...
  facet_wrap(~variable, scales = "free_y", nrow = 4, ncol = 2) +
...
**Figure 1.** Differences in variables measured by Oxiplex and associated α values among patient groups. THC levels were different between all three groups (Kruskal-Wallis adjusted p-value = 3.25x10^-6^; Dunn Test post-hoc adjusted p-values: CM vs HV = 1.66x10^-6^, CM vs UM = .032, HV vs UM = .033). Levels of cerebral O~2~ saturation did not differ among groups (Kruskal-Wallis adjusted p-value = .07). Note: removed one outlier from Deoxy Hb α (3.52).

### Figure 2: Logistic regression on CM vs UM
