...
df_hbox_trim <- df_hbox %>% 
  ...
  dplyr::rename("subject_id" = "Subject.ID..NIAID.", 
                ...
                "[Hb]" = "cerebral.hbdeox", "[Hb]α" = "cerebral.hbdeox.alpha", 
                "[HbO2]" = "cerebral.hbox", "[HbO2]α" = "cerebral.hbox.alpha",  
...
df_hbox_voi <- df_hbox_impute %>% 
  dplyr::select(c(Status, `[HbO2]`, `[HbO2]α`, `[Hb]`, `[Hb]α`, `HbO2%`, `HbO2% α`, `[HbT]`, `[HbT]α`)) %>% 
  dplyr::filter(`[Hb]α` < 3)

df_hbox_voi_long <- df_hbox_voi %>% 
  pivot_longer(2:9, names_to = "variable", values_to = "value") %>%
  mutate(variable = factor(variable, levels = c("[HbO2]", "[HbO2]α",   
                                                 "[Hb]", "[Hb]α", 
                                                "HbO2%", "HbO2% α", 
                                                "[HbT]", "[HbT]α"))) 
...
df_hbox_voi_long %>% dplyr::filter(variable %in% c("[HbO2]", "[Hb]", "HbO2%", "[HbT]")) %>%
...
df_hbox_voi_long %>% dplyr::filter(variable %in% c("[HbO2]α", "[Hb]α", "HbO2% α", "[HbT]α")) %>% 
...
df_hbox_reg <- df_hbox_impute %>% 
  dplyr::select(c(Status, `Hematocrit (%)`, `Lactate (mmol/L)`, `[HbO2]`, `[HbO2]α`, `[Hb]`, `[Hb]α`, `HbO2%`, `HbO2% α`, `[HbT]`, `[HbT]α`)) %>% 
  dplyr::filter(`[Hb]α` < 3)
...
f_plot_cor <- function(mat) {
  ...
  melt(mat) %>% 
    ...
    mutate_if(is.factor, factor, levels = c("Lactate (mmol/L)", "Hematocrit (%)", "[HbO2]", "[Hb]", "HbO2%", "[HbT]", "[HbO2]α", "[Hb]α", "HbO2% α", "[HbT]α")) %>% 
...
df_hbox_reg_long <- df_hbox_reg %>% 
  ...
  mutate(variable = factor(variable, levels = c("Lactate (mmol/L)", "Hematocrit (%)", "[HbO2]", "[HbO2]α", "[Hb]", "[Hb]α", "HbO2%", "HbO2% α" ,"[HbT]", "[HbT]α")))
