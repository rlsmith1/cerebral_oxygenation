...
df_hbox_trim <- df_hbox %>% 
  ...
  dplyr::rename("subject_id" = "Subject.ID..NIAID.", 
                ...
                "HbO2%" = "cerebral.sat", "HbO2% α" = "cerebral.sat.alpha", 
                "[HbT]" = "cerebral.thc", "[HbT]α" = "cerebral.thc.alpha") 
...
df_hbox_voi <- df_hbox_impute %>% 
  dplyr::select(c(Status, `Oxy Hb`, `Oxy Hb α`, `Deoxy Hb`, `Deoxy Hb α`, `HbO2%`, `HbO2% α`, `[HbT]`, `[HbT]α`)) %>% 
...
df_hbox_voi_long %>% dplyr::filter(variable %in% c("Oxy Hb", "Deoxy Hb", "HbO2%", "[HbT]")) %>%
...
# [HbT]
kruskal.test(`[HbT]` ~ Status, data = df_hbox_voi)
dunnTest(`[HbT]` ~ Status, data = df_hbox_voi, method = "bh")

# HbO2%
kruskal.test(`HbO2%` ~ Status, data = df_hbox_voi)

```

**Figure 1.** Shown are the variables measured by Oxiplex, plotted by patient status [HbT] levels were different between all three groups (Kruskal-Wallis adjusted p-value = 3.25x10^-6^; Dunn Test post-hoc adjusted p-values: CM vs HV = 1.66x10^-6^, CM vs UM = .032, HV vs UM = .033). Levels of cerebral O~2~ saturation did not differ among groups (Kruskal-Wallis adjusted p-value = .07). 
...
df_hbox_voi_long %>% dplyr::filter(variable %in% c("Oxy Hb α", "Deoxy Hb α", "HbO2% α", "[HbT]α")) %>% 
  ...
  geom_hline(yintercept = 0.4, alpha = 0.4, lty = 2) +
  geom_hline(yintercept = 0.6, alpha = 0.4, lty = 2) +
...
# [HbT]α
kruskal.test(`[HbT]α` ~ Status, data = df_hbox_voi)
dunnTest(`[HbT]α` ~ Status, data = df_hbox_voi, method = "bh")

```

**Figure 2** Shown are the alpha values associated with variables measured by the Oxiplex, plotted by patient status. Note: One outlier was removed from Deoxy Hb α (3.52).

### Figure 3: Logistic regression model on CM vs UM

#### A. PCA
```{r fig3a}
# include variables of interest ([HbT], [HbT] alpha, and hematocrit), remove healthy volunteers from analysis
...
# select [HbT], [HbT] alpha, and hematocrit
  df_hbox_model <- df_hbox_impute %>% 
    dplyr::select(c(2,4:5,19:22)) %>% 
    mutate(`1/10 * [HbT]α` = `[HbT]α`/10, `[HbT] (nM)` = `[HbT]`/1000) %>% 
    dplyr::select(-c(`[HbT]α`, `[HbT]`)) %>% 
    mutate(Status = as.factor(ifelse(Status == "CM", 1, 0))) %>% 
    
    dplyr::select(-c(`HbO2%`, `HbO2% α`, `Lactate (mmol/L)`))
...
### Figure 4: Relationships among [HbT], [HbT] alpha, and hematocrit
```{r fig4}

df_hbox_impute %>% filter(Status != "HV") %>% 
  
  ggplot(aes(x = `[HbT]`, y = `Hematocrit (%)`)) +
  geom_point(aes(color = Status, size = `[HbT]α`), alpha = 0.7) +
...
# create df of potential regressors
df_hbox_reg <- df_hbox_impute %>% 
  dplyr::select(c(Status, `Hematocrit (%)`, `Lactate (mmol/L)`, `Oxy Hb`, `Oxy Hb α`, `Deoxy Hb`, `Deoxy Hb α`, `HbO2%`, `HbO2% α`, `[HbT]`, `[HbT]α`)) %>% 
  dplyr::filter(`Deoxy Hb α` < 3)

m_cor_hv <- df_hbox_reg %>% f_calc_cor(status = "HV")

m_cor_um <- df_hbox_reg %>% f_calc_cor(status = "UM")

m_cor_cm <- df_hbox_reg %>% f_calc_cor(status = "CM")
...
f_plot_cor <- function(mat) {
  ...
  melt(mat) %>% 
    ...
    mutate_if(is.factor, factor, levels = c("Lactate (mmol/L)", "Hematocrit (%)", "Oxy Hb", "Deoxy Hb", "HbO2%", "[HbT]", "Oxy Hb α", "Deoxy Hb α", "HbO2% α", "[HbT]α")) %>% 
...
df_hbox_reg_long <- df_hbox_reg %>% 
  pivot_longer(2:11, names_to = "variable", values_to = "value") %>% 
  mutate(variable = factor(variable, levels = c("Lactate (mmol/L)", "Hematocrit (%)", "Oxy Hb", "Oxy Hb α", "Deoxy Hb", "Deoxy Hb α", "HbO2%", "HbO2% α" ,"[HbT]", "[HbT]α")))

# plot as individual regression curves
df_hbox_reg_long %>% mutate(Status = as.numeric(ifelse(Status == "CM", 1, 0))) %>% 
