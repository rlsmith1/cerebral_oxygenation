...
dunnTest(avg_Hb_conc ~ Status, data = df_hbox, method = "bh")
...
**Figure 1.** Shown are the variables measured by Oxiplex, plotted by patient status. Cerebral tissue hemoglobin concentration levels were different between cerebral malaria patients and the other two groups, but did not differ between uncomplicated malaria patients and healthy volunteers (Kruskal-Wallis adjusted p-value = 5.00x10^-7^; Dunn Test post-hoc adjusted p-values: CM vs HV = 3.22x10^-6^, CM vs UM = 2.76x10^-4^, HV vs UM = 0.87). Levels of cerebral hemoglobin O~2~ saturation did not differ among groups (Kruskal-Wallis adjusted p-value = 0.28). 
...
dunnTest(Hb_conc_second_alpha ~ Status, data = df_hbox, method = "bh")

# hemoglobin oxygen saturation alpha
kruskal.test(Hb_o2sat_second_alpha ~ Status, data = df_hbox)
dunnTest(Hb_o2sat_second_alpha ~ Status, data = df_hbox, method = "bh")

```

**Figure 2.** Shown are the alpha values associated with the hemoglobin concentration and oxygen saturation signals measured by Oxiplex. Differences among cerebral tissue hemoglobin concentration alpha values were significant by Kruskal-Wallis (adjusted p-value = 1.57x10^-4^), and differences between all groups were significant by post-hoc Dunn Test (adjusted p-values: CM vs HV = 0.03, CM vs UM = 1.65x10^-4^, HV vs UM = 0.04). Cerebral hemoglobin O~2~ saturation alpha values differed between cerebral malaria patients and the other two patient groups (Kruskal-Wallis adjusted p-value = 2.71x10^-5^; Dunn Test post-hoc adjusted p-values: CM vs HV = 5.18x10^-4^, CM vs UM = 4.44x10^-4^, HV vs UM = 0.23). 
...
### Figure 4: Relationships among hemoglobin concentration, hemoglobin concentration alpha, and hematocrit
```{r fig4}
